  ...
  describe 'setting the database type' do
    ...
    context 'with Postgres' do
      ...
      it 'uses tls if require_tls is true' do
        data_storage_yaml = '{ type: "postgres", host: "my_host", port: 1234, database: "my_db_name", username: "my-username", password: "my-password", require_tls: true, tls_ca: {certificate: cert, private_key: key} }'
      ...
      it 'does not use tls if require_tls is false' do
        data_storage_yaml = '{ type: "postgres", host: "my_host", port: 1234, database: "my_db_name", username: "my-username", password: "my-password", require_tls: false }'
        result = render_erb_to_hash(data_storage_yaml)
        url = parse_database_url(result['spring']['datasource']['url'])
