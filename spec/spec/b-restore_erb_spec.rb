...
def render_b_restore_erb(dbtype="postgres")
  option_yaml = <<-EOF
        properties:
          credhub:
            data_storage:
...
RSpec.describe "the template" do
  ...
  context "when db is not postgres" do
    it "logs that it skips this restore," do
      result = render_b_restore_erb("NOT_PG")
