...
openssl pkcs12 -export -in $CERT_FILE -inkey $PRIVATE_KEY_FILE -out cert.p12 \
        -name $CERT_ALIAS -password pass:k0*l*s3cur1tyr0ck$

$JDK_HOME/bin/keytool -importcert -file cert.p12 -keystore $TRUST_STORE_FILE \
        -storepass $KEYSTORE_PASSWORD -noprompt -alias $CERT_ALIAS
